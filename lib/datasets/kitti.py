...
# if os.path.exists('/private/personal/pengliang'):
#     tmp_p = '/private/personal/pengliang'
# elif os.path.exists('/private/pengliang'):
#     tmp_p = '/private/pengliang'
# elif os.path.exists('/private_data/personal/pengliang'):
#     tmp_p = '/private_data/personal/pengliang'
# else:
#     print('path error private')
#
...
'''private has been damaged'''
train_3D_mapping_file_path = '/pvc_user/pengliang/LPCG/data/kitti/data_file/train_mapping.txt'
kitti_3D_rand_file_path = '/pvc_user/pengliang/LPCG/data/kitti/data_file/train_rand.txt'
train_3D_file_path = '/pvc_user/pengliang/LPCG/data/kitti/KITTI3D/ImageSets/train.txt'
val_3D_file_path = '/pvc_user/pengliang/LPCG/data/kitti/KITTI3D/ImageSets/val.txt'
kitti_raw_data_dir = '/pvc_data/personal/pengliang/kitti_raw'
...
class KITTI(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        # '''lidar depth'''
        # lidar_d_path = cv.imread('/pvc_data/personal/pengliang/KITTI3D/training/lidar_depth/{:0>6}.png'.format(index), -1) / 256.
        # lidar_d = Image.fromarray(lidar_d_path)
        ...
        # flow_pair_path = []
        # save_flow_dir = '/private/personal/pengliang/RAFT/KITTI3D_flow/training'
        # for ind in range(7481):
        #     seq = self.mapping[ind]
        #     cur_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
        #                               'image_02/data/{:0>10}.png'.format(int(seq[2])))
        #     pre_1_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
        #                               'image_02/data/{:0>10}.png'.format(int(seq[2]) - 1))
        #     pre_2_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
        #                               'image_02/data/{:0>10}.png'.format(int(seq[2]) - 2))
        ...
        #     flow_2_path = ind*2+0
        #     flow_1_path = ind*2+1
        #     flow_pair_path.append([pre_2_path, cur_path, os.path.join(save_flow_dir, '{:0>6}.png'.format(flow_2_path))])
        #     flow_pair_path.append([pre_1_path, cur_path, os.path.join(save_flow_dir, '{:0>6}.png'.format(flow_1_path))])
        # np.savetxt('/private/personal/pengliang/RAFT/kitti3d_training_flow_seq.txt', np.array(flow_pair_path), fmt='%s')


        # flow_pair_path = []
        # save_flow_dir = '/private/personal/pengliang/RAFT/KITTI3D_flow/testing'
        # test_img_dir = '/private/personal/pengliang/KITTI3D/testing/image_2/'
        # seq_img_dir = '/private/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/testing/prev_2'
        # for ind in range(7518):
        #     cur_path = os.path.join(test_img_dir, '{:0>6}.png'.format(ind))
        #     pre_1_path = os.path.join(seq_img_dir, '{:0>6}_01.png'.format(ind))
        #     pre_2_path = os.path.join(seq_img_dir, '{:0>6}_02.png'.format(ind))
        #     if not os.path.exists(pre_1_path):
        #         pre_1_path = cur_path
        #     if not os.path.exists(pre_2_path):
        #         pre_2_path = cur_path
        ...
        #     flow_pair_path.append([pre_2_path, cur_path, os.path.join(save_flow_dir, '{:0>6}.png'.format(flow_2_path))])
        #     flow_pair_path.append([pre_1_path, cur_path, os.path.join(save_flow_dir, '{:0>6}.png'.format(flow_1_path))])
        # np.savetxt('/private/personal/pengliang/RAFT/kitti3d_testing_flow_seq.txt', np.array(flow_pair_path), fmt='%s')

        if self.split!='test':
            seq = self.mapping[index]
            pre_1_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1], 'image_02/data/{:0>10}.png'.format(int(seq[2])-1))
            pre_2_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1], 'image_02/data/{:0>10}.png'.format(int(seq[2])-2))
        else:
            pre_1_path = os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/testing/prev_2', '{:0>6}_01.png'.format(index))
            pre_2_path = os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/testing/prev_2', '{:0>6}_02.png'.format(index))

        if not os.path.exists(pre_1_path):
            pre_1 = img.copy()
        else:
            pre_1 = Image.open(pre_1_path)
        if not os.path.exists(pre_2_path):
            pre_2 = pre_1.copy()
        else:
            pre_2 = Image.open(pre_2_path)

        if self.split!='test':
            # if os.path.exists('/private_data'):
            #     # pre_2_flow = np.load(os.path.join('/private_data/personal/pengliang/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index*2+0)))
            #     # pre_1_flow = np.load(os.path.join('/private_data/personal/pengliang/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index*2+1)))
            #     pre_2_flow = np.load(os.path.join('/private_data/personal/pengliang/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index*2+0)))
            #     pre_1_flow = np.load(os.path.join('/private_data/personal/pengliang/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index*2+1)))
            # else:
            #     # pre_2_flow = np.load(os.path.join('/private/personal/pengliang/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index*2+0)))
            #     # pre_1_flow = np.load(os.path.join('/private/personal/pengliang/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index*2+1)))
            #     pre_2_flow = np.load(os.path.join('/private/personal/pengliang/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index*2+0)))
            #     pre_1_flow = np.load(os.path.join('/private/personal/pengliang/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index*2+1)))
            pre_2_flow = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index * 2 + 0)))
            pre_1_flow = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/training', '{:0>6}.npy'.format(index * 2 + 1)))
            pre_2_flow_bk = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index * 2 + 0)))
            pre_1_flow_bk = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/training', '{:0>6}.npy'.format(index * 2 + 1)))
        else:
            pre_2_flow = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/testing', '{:0>6}.npy'.format(index*2+0)))
            pre_1_flow = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/testing', '{:0>6}.npy'.format(index*2+1)))
            pre_2_flow_bk = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/testing', '{:0>6}.npy'.format(index * 2 + 0)))
            pre_1_flow_bk = np.load(os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/testing', '{:0>6}.npy'.format(index * 2 + 1)))


        pre_2_flow_u, pre_2_flow_v = Image.fromarray(pre_2_flow[..., 0]), Image.fromarray(pre_2_flow[..., 1])
        pre_1_flow_u, pre_1_flow_v = Image.fromarray(pre_1_flow[..., 0]), Image.fromarray(pre_1_flow[..., 1])
        pre_2_flow_u_bk, pre_2_flow_v_bk = Image.fromarray(pre_2_flow_bk[..., 0]), Image.fromarray(pre_2_flow_bk[..., 1])
        pre_1_flow_u_bk, pre_1_flow_v_bk = Image.fromarray(pre_1_flow_bk[..., 0]), Image.fromarray(pre_1_flow_bk[..., 1])
        ####################################################################
        ...
        c = datetime.now()
        # print('data loading read', c-a, c, a)
        ...
        if self.data_augmentation:
            if np.random.random() < self.random_flip:
                ...
                # lidar_d = lidar_d.transpose(Image.FLIP_LEFT_RIGHT)

                pre_1 = pre_1.transpose(Image.FLIP_LEFT_RIGHT)
                pre_2 = pre_2.transpose(Image.FLIP_LEFT_RIGHT)
                pre_2_flow_u, pre_2_flow_v = pre_2_flow_u.transpose(Image.FLIP_LEFT_RIGHT), pre_2_flow_v.transpose(Image.FLIP_LEFT_RIGHT)
                pre_1_flow_u, pre_1_flow_v = pre_1_flow_u.transpose(Image.FLIP_LEFT_RIGHT), pre_1_flow_v.transpose(Image.FLIP_LEFT_RIGHT)
                pre_2_flow_u_bk, pre_2_flow_v_bk = pre_2_flow_u_bk.transpose(Image.FLIP_LEFT_RIGHT), pre_2_flow_v_bk.transpose(Image.FLIP_LEFT_RIGHT)
                pre_1_flow_u_bk, pre_1_flow_v_bk = pre_1_flow_u_bk.transpose(Image.FLIP_LEFT_RIGHT), pre_1_flow_v_bk.transpose(Image.FLIP_LEFT_RIGHT)

                pre_2_flow_u = np.array(pre_2_flow_u)
                pre_1_flow_u = np.array(pre_1_flow_u)
                pre_2_flow_u = -pre_2_flow_u
                pre_1_flow_u = -pre_1_flow_u
                pre_2_flow_u = Image.fromarray(pre_2_flow_u)
                pre_1_flow_u = Image.fromarray(pre_1_flow_u)

                pre_2_flow_u_bk = np.array(pre_2_flow_u_bk)
                pre_1_flow_u_bk = np.array(pre_1_flow_u_bk)
                pre_2_flow_u_bk = -pre_2_flow_u_bk
                pre_1_flow_u_bk = -pre_1_flow_u_bk
                pre_2_flow_u_bk = Image.fromarray(pre_2_flow_u_bk)
                pre_1_flow_u_bk = Image.fromarray(pre_1_flow_u_bk)
        ...
        # lidar_d = lidar_d.transform(tuple(self.resolution.tolist()),
        #                     method=Image.AFFINE,
        #                     data=tuple(trans_inv.reshape(-1).tolist()),
        #                     resample=Image.NEAREST)

        pre_1 = pre_1.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_2 = pre_2.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_1_flow_u = pre_1_flow_u.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_1_flow_v = pre_1_flow_v.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_2_flow_u = pre_2_flow_u.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_2_flow_v = pre_2_flow_v.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)

        pre_1_flow_u_bk = pre_1_flow_u_bk.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_1_flow_v_bk = pre_1_flow_v_bk.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_2_flow_u_bk = pre_2_flow_u_bk.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)
        pre_2_flow_v_bk = pre_2_flow_v_bk.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)

        pre_1_flow_u, pre_1_flow_v = np.array(pre_1_flow_u), np.array(pre_1_flow_v)
        pre_1_flow_u *= self.resolution[0] / crop_size[0]
        pre_1_flow_v *= self.resolution[1] / crop_size[1]
        pre_2_flow_u, pre_2_flow_v = np.array(pre_2_flow_u), np.array(pre_2_flow_v)
        pre_2_flow_u *= self.resolution[0] / crop_size[0]
        pre_2_flow_v *= self.resolution[1] / crop_size[1]

        pre_1_flow_u_bk, pre_1_flow_v_bk = np.array(pre_1_flow_u_bk), np.array(pre_1_flow_v_bk)
        pre_1_flow_u_bk *= self.resolution[0] / crop_size[0]
        pre_1_flow_v_bk *= self.resolution[1] / crop_size[1]
        pre_2_flow_u_bk, pre_2_flow_v_bk = np.array(pre_2_flow_u_bk), np.array(pre_2_flow_v_bk)
        pre_2_flow_u_bk *= self.resolution[0] / crop_size[0]
        pre_2_flow_v_bk *= self.resolution[1] / crop_size[1]
        ...
        pre_1 = np.array(pre_1).astype(np.float32) / 255.0
        pre_1 = (pre_1 - self.mean) / self.std
        pre_1 = pre_1.transpose(2, 0, 1)  # C * H * W

        pre_2 = np.array(pre_2).astype(np.float32) / 255.0
        pre_2 = (pre_2 - self.mean) / self.std
        pre_2 = pre_2.transpose(2, 0, 1)  # C * H * W
        ...
        inputs = {'img': img,
                  'pre_1': pre_1,
                  'pre_2': pre_2,
                  'pre_1_flow': np.transpose(np.stack([pre_1_flow_u, pre_1_flow_v], axis=2), [2, 0, 1]),
                  'pre_2_flow': np.transpose(np.stack([pre_2_flow_u, pre_2_flow_v], axis=2), [2, 0, 1]),
                  'pre_1_flow_bk': np.transpose(np.stack([pre_1_flow_u_bk, pre_1_flow_v_bk], axis=2), [2, 0, 1]),
                  'pre_2_flow_bk': np.transpose(np.stack([pre_2_flow_u_bk, pre_2_flow_v_bk], axis=2), [2, 0, 1]),

                  # 't_cy': t_cy.astype(np.float32)
                  'instr': instr,
