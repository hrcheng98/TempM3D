...
train_3D_mapping_file_path = tmp_p + '/KITTI3D/train_mapping.txt'
kitti_3D_rand_file_path = tmp_p + '/KITTI3D/train_rand.txt'
train_3D_file_path = tmp_p + '/KITTI3D/ImageSets/train.txt'
val_3D_file_path = tmp_p + '/KITTI3D/ImageSets/val.txt'
...
class KITTIFlow(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        a = datetime.now()
        # tt = np.load('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all/training/000000.npy')
        # b = datetime.now()
        #
        # print(b-a, a, b)
        ...
        if self.split != 'test':
            ...
            #     '/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/training/prev_2',
            #     '{:0>6}_01.png'.format(index))
            # pre_2_path = os.path.join(
            #     '/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/training/prev_2',
            #     '{:0>6}_02.png'.format(index))
            # cur_path = os.path.join(
            #     '/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/training/prev_2',
            #     '{:0>6}_00.png'.format(index))

            '''for original RAFT 1/1'''
            pre_1_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
                                      'image_02/data/{:0>10}.png'.format(max(int(seq[2]) - 1, 0)))
            pre_2_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
                                      'image_02/data/{:0>10}.png'.format(max(int(seq[2]) - 2, 0)))
            pre_3_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
                                      'image_02/data/{:0>10}.png'.format(max(int(seq[2]) - 3, 0)))
            pre_4_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
                                      'image_02/data/{:0>10}.png'.format(max(int(seq[2]) - 4, 0)))
            ...
            # pre_3_path = os.path.join(kitti_raw_data_dir, seq[0], seq[1],
            #                           'image_02/data/{:0>10}.png'.format(int(seq[2]) - 3))
        else:
            pre_1_path = os.path.join(
                '/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/testing/prev_2',
                '{:0>6}_01.png'.format(index))
            pre_2_path = os.path.join(
                '/pvc_data/personal/pengliang/KITTI3D/kitti_3d_pre/data_object_prev_2/testing/prev_2',
                '{:0>6}_02.png'.format(index))
        ...
        if not os.path.exists(pre_3_path):
            pre_3 = pre_2.copy()
            print(pre_3_path)
        else:
            pre_3 = cv.imread(pre_3_path)
        if not os.path.exists(pre_4_path):
            pre_4 = pre_3.copy()
            print(pre_4_path)
        else:
            pre_4 = cv.imread(pre_4_path)
        ...
        # pre_2_flow = np.load(
        #     os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/{}'.format(cur_split),
        #                  '{:0>6}.npy'.format(index * 2 + 0)))
        # pre_1_flow = np.load(
        #     os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow/{}'.format(cur_split),
        #                  '{:0>6}.npy'.format(index * 2 + 1)))
        # pre_2_flow_bk = np.load(
        #     os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/{}'.format(cur_split),
        #                  '{:0>6}.npy'.format(index * 2 + 0)))
        # pre_1_flow_bk = np.load(
        #     os.path.join('/pvc_data/personal/pengliang/private_data/RAFT/KITTI3D_flow_bk/{}'.format(cur_split),
        #                  '{:0>6}.npy'.format(index * 2 + 1)))


        # pre_2_flow_u = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all/{}'.format(cur_split),
        #                  '{:0>6}_u.png'.format(index * 2 + 0)), -1) / 10. - 1000.
        # pre_1_flow_u = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all/{}'.format(cur_split),
        #                  '{:0>6}_u.png'.format(index * 2 + 1)), -1) / 10. - 1000.
        # pre_2_flow_v = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all/{}'.format(cur_split),
        #                  '{:0>6}_v.png'.format(index * 2 + 0)), -1) / 10. - 1000.
        # pre_1_flow_v = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all/{}'.format(cur_split),
        #                  '{:0>6}_v.png'.format(index * 2 + 1)), -1) / 10. - 1000.
        # pre_2_flow = np.stack([pre_2_flow_u, pre_2_flow_v], axis=2)
        # pre_1_flow = np.stack([pre_1_flow_u, pre_1_flow_v], axis=2)
        #
        # pre_2_flow_u_bk = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all_bk/{}'.format(cur_split),
        #                  '{:0>6}_u.png'.format(index * 2 + 0)), -1) / 10. - 1000.
        # pre_1_flow_u_bk = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all_bk/{}'.format(cur_split),
        #                  '{:0>6}_u.png'.format(index * 2 + 1)), -1) / 10. - 1000.
        # pre_2_flow_v_bk = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all_bk/{}'.format(cur_split),
        #                  '{:0>6}_v.png'.format(index * 2 + 0)), -1) / 10. - 1000.
        # pre_1_flow_v_bk = cv.imread(
        #     os.path.join('/pvc_data/personal/pengliang/KITTI3D/kitti_flow_all_bk/{}'.format(cur_split),
        #                  '{:0>6}_v.png'.format(index * 2 + 1)), -1) / 10. - 1000.
        ...
        if self.split != 'test':
            # '''for 1/4 downsample'''
            # pre_2_1_flow = pre_1_path.replace('training', 'training_DIP_flow')
            # pre_1_0_flow = cur_path.replace('training', 'training_DIP_flow')
            # # pre_2_1_flow = pre_1_path.replace('training', 'training_DIP_flow_v2')
            # # pre_1_0_flow = cur_path.replace('training', 'training_DIP_flow_v2')


            '''for original RAFT 1/1'''
            pre_4_3_flow = pre_3_path.replace('kitti_raw', 'kitti_raw_flow')
            pre_3_2_flow = pre_2_path.replace('kitti_raw', 'kitti_raw_flow')
            pre_2_1_flow = pre_1_path.replace('kitti_raw', 'kitti_raw_flow')
            pre_1_0_flow = cur_path.replace('kitti_raw', 'kitti_raw_flow')
        else:
            pre_2_1_flow = pre_1_path.replace('testing', 'testing_flow')
            pre_1_0_flow = cur_path.replace('testing', 'testing_flow')

        if not os.path.exists(pre_2_1_flow.replace('.png', '_u.png')):
            pre_2_1_flow = pre_1_0_flow
        if not os.path.exists(pre_3_2_flow.replace('.png', '_u.png')):
            pre_3_2_flow = pre_2_1_flow
        if not os.path.exists(pre_4_3_flow.replace('.png', '_u.png')):
            pre_4_3_flow = pre_3_2_flow
        ...
        pre_3_flow_u = cv.imread(pre_3_2_flow.replace('.png', '_u.png'), -1) / 10. - 1000.
        pre_3_flow_v = cv.imread(pre_3_2_flow.replace('.png', '_v.png'), -1) / 10. - 1000.
        pre_3_flow = np.stack([pre_3_flow_u, pre_3_flow_v], axis=2)

        pre_3_flow_u_bk = cv.imread(pre_3_2_flow.replace('.png', '_bk_u.png'), -1) / 10. - 1000.
        pre_3_flow_v_bk = cv.imread(pre_3_2_flow.replace('.png', '_bk_v.png'), -1) / 10. - 1000.
        pre_3_flow_bk = np.stack([pre_3_flow_u_bk, pre_3_flow_v_bk], axis=2)

        pre_4_flow_u = cv.imread(pre_4_3_flow.replace('.png', '_u.png'), -1) / 10. - 1000.
        pre_4_flow_v = cv.imread(pre_4_3_flow.replace('.png', '_v.png'), -1) / 10. - 1000.
        pre_4_flow = np.stack([pre_4_flow_u, pre_4_flow_v], axis=2)

        pre_4_flow_u_bk = cv.imread(pre_4_3_flow.replace('.png', '_bk_u.png'), -1) / 10. - 1000.
        pre_4_flow_v_bk = cv.imread(pre_4_3_flow.replace('.png', '_bk_v.png'), -1) / 10. - 1000.
        pre_4_flow_bk = np.stack([pre_4_flow_u_bk, pre_4_flow_v_bk], axis=2)



        # pre_3_2_flow = pre_1_path.replace('kitti_raw', 'kitti_raw_flow')
        # if not os.path.exists(pre_3_2_flow.replace('.png', '_u.png')):
        #     pre_3_2_flow = pre_2_1_flow
        # pre_3_flow_u = cv.imread(pre_3_2_flow.replace('.png', '_u.png'), -1) / 10. - 1000.
        # pre_3_flow_v = cv.imread(pre_3_2_flow.replace('.png', '_v.png'), -1) / 10. - 1000.
        ...
        # '''for 1/4 downsample'''
        # down_h, down_w = img_size[1] // 4, img_size[0] // 4
        # pad_ht = (((down_h // 8) + 1) * 8 - down_h) % 8
        # pad_wd = (((down_w // 8) + 1) * 8 - down_w) % 8
        ...
        # pre_2_flow = cv.resize(pre_2_flow, (img_size[0], img_size[1]))
        # pre_1_flow = cv.resize(pre_1_flow, (img_size[0], img_size[1]))
        # pre_2_flow_bk = cv.resize(pre_2_flow_bk, (img_size[0], img_size[1]))
        # pre_1_flow_bk = cv.resize(pre_1_flow_bk, (img_size[0], img_size[1]))
        ...
        pre_4_flow = pre_4_flow[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_3_flow = pre_3_flow[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_2_flow = pre_2_flow[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_1_flow = pre_1_flow[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_4_flow_bk = pre_4_flow_bk[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_3_flow_bk = pre_3_flow_bk[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_2_flow_bk = pre_2_flow_bk[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        pre_1_flow_bk = pre_1_flow_bk[pad_v[2]:pad_v[2] + img_size[1], pad_v[0]:pad_v[0] + img_size[0], :]
        ...
        cat_all = np.concatenate([img, 
                                  pre_1, pre_2,
                                  pre_1_flow, pre_2_flow, pre_1_flow_bk, pre_2_flow_bk,
                                  pre_3, pre_4,
                                  pre_3_flow, pre_4_flow, pre_3_flow_bk, pre_4_flow_bk,
                                  d[..., np.newaxis]
                                  ], 2).astype(np.float32)
        # cat_all = np.concatenate([img,
        #                           d[..., np.newaxis]
        ...
        if self.data_augmentation:
            if np.random.random() < self.random_flip:
                ...
                cat_all[..., [9, 11, 13, 15, 23, 25, 27, 29]] = -cat_all[..., [9, 11, 13, 15, 23, 25, 27, 29]]
        ...
        cat_all[..., [9, 11, 13, 15, 23, 25, 27, 29]] = cat_all[..., [9, 11, 13, 15, 23, 25, 27, 29]] * (self.resolution[0] / crop_size[0])
        cat_all[..., [10, 12, 14, 16, 24, 26, 28, 30]] = cat_all[..., [10, 12, 14, 16, 24, 26, 28, 30]] * (self.resolution[1] / crop_size[1])
        ...
        pre_3 = np.transpose((cat_all[..., 17:20].astype(np.float32) / 255.0 - self.mean) / self.std, [2, 0, 1])
        pre_4 = np.transpose((cat_all[..., 20:23].astype(np.float32) / 255.0 - self.mean) / self.std, [2, 0, 1])
        ...
        inputs = {'img': img,
                  ...
                  'pre_3_flow': np.transpose(cat_all[..., [23, 24]], [2, 0, 1]),
                  'pre_4_flow': np.transpose(cat_all[..., [25, 26]], [2, 0, 1]),
                  'pre_3_flow_bk': np.transpose(cat_all[..., [27, 28]], [2, 0, 1]),
                  'pre_4_flow_bk': np.transpose(cat_all[..., [29, 30]], [2, 0, 1]),
                  'instr': instr
